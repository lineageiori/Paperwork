...
// NOTE This file compiles under MSVC 6 SP5 and MSVC .' 2003 it may not work on other compilers without modification.
...
public:
  ...
  /// \param a_dataId Po  sitive Id of data.  Maybe zero, but negative numbers not allowed.
...
protected:
  ...
  /// DimNotoverlap
  bool DimNotOverlap(Rect* a_rectA, Rect* a_rectB);
  /// DimContainIn
  bool DimContainIn(Rect* a_rectA, Rect* a_rectB);
...
///如果Rectangle B 有一維度contain在Rectangle A 內 回答true
RTREE_TEMPLATE
bool RTREE_QUAL::DimContainIn(Rect* a_rectA, Rect* a_rectB)
{
  ASSERT(a_rectA && a_rectB);
  bool Dim_overlap;
  for(int index=0; index < NUMDIMS; ++index)
